function run_lick_mega_pipeline_new()
% LICKING MEGA-PIPELINE (all-in-one; toolbox-free fallbacks)
% -------------------------------------------------------------------------
% Updated for NEW COHORT + NEW TIMELINE BINS (Pre/During/Post/Withdrawal/Reexp)
%
% New cohort convention:
%   mouse_key contains cage (4 digits) + color (mouse ID), e.g. "6100 red"
%   Group: a=Active, p=Passive; Sex: f/m; PairID defined below.
%
% Timeline:
%   D1-2   habituation (ignored by bins; still allowed in raw table)
%   Pre        D3-5     (water PR)
%   During     D6-10    (passive training morphine; active has PR, passive may not)
%   Post       D11-13   (all mice morphine PR)
%   Withdrawal D14-16   (water PR)
%   Reexposure D17-18   (morphine PR)
%
% Optional exclusion of "less reliable switch days":
%   default unreliableDays = [4 6 11 14 17]
%   (edit below if you want different)
%
...
opts.SEQ_models        = true;   % (placeholder; not expanded here)
opts.REQ_align         = true;   % 4A–B
opts.MIXED_models      = true;   % GLM approximations

% thresholds & params
PLOT_MAX_MOUSE_EXAMPLES = 12;
BIN_50MS = 0.05;
BOUT_WITHIN_IEI = 0.25;
BOUT_GAP_IEI    = 0.5;
RHYTHM_BAND     = [3 12];
PEAK_SEARCH     = [6 10];
PHASE_BAND      = [5 9];
...
% Timeline bin settings
unreliableDays = [4 6 11 14 17];   % EDIT HERE if you don't want e.g. day17 excluded
USE_CLEAN_BINS_FOR_PLOTS = false;  % set true if you want all plots to use PeriodClean

%% --------------------- cohort definition (NEW) --------------------------
% Canonical key: "#### color" where color in {red, orange, black, white}
% Pair mapping per your description:
%   Pair1: 6100 black (A) vs 6100 orange, red (P)
%   Pair2: 0911 red (A) vs 0911 orange (P)
%   Pair3: 0911 white (A) vs 0911 black (P)
%   Pair4: 0910 black (A) vs 0910 orange, red (P)
%   Pair5: 6099 orange (A) vs 6099 red (P)
%   Pair6: 6099 black (A) vs 6099 white (P)  (white died after D13; handled automatically)

cohort = build_new_cohort_table();  % table with CanonKey, Cage, Color, Sex, Group, PairID
cohortMap = containers.Map(string(cohort.CanonKey), num2cell(1:height(cohort)));
...
%% --------------------- exclusions & basics ------------------------------
% hard exclusions (kept; harmless if not present)
...
for c = 1:numel(reqCols)
    assert(ismember(reqCols{c},T.Properties.VariableNames), 'Missing %s', reqCols{c});
end
...
% output dir
outDir = fullfile(runDir,'figs','lick_MEGA'); 
if ~exist(outDir,'dir'), mkdir(outDir); end

% requirement & reward columns if exist
reqCol    = pickRequirement(T);
rewardCol = pickRewardTTL(T);

%% --------------------- attach cohort metadata (NEW) ---------------------
% Adds per-row: CanonKey, Cage, Color, Sex, Group, PairID
[T, cohortAssignSummary] = attach_cohort_metadata(T, cohort, cohortMap);
writetable(cohortAssignSummary, fullfile(outDir,'cohort_assignment_summary.csv'));
...
rng(RANDSEED);
for i=1:height(Sess)
    r = (string(T.mouse_key)==string(Sess.mouse_key(i))) & ...
        (T.day_index==Sess.day_index(i)) & (T.session_idx==Sess.session_idx(i));
...
% attach cohort group/pair/sex to Sess using mouse_key parsing
Sess = attach_mouselevel_metadata_to_keys(Sess, cohort, cohortMap);

% timeline labels
Sess.Period      = categorical(period_label_vec(Sess.day_index, false, unreliableDays), ...
                               ["Pre","During","Post","Withdrawal","Reexposure","<undef>"]);
Sess.PeriodClean = categorical(period_label_vec(Sess.day_index, true,  unreliableDays), ...
                               ["Pre","During","Post","Withdrawal","Reexposure","<excluded>","<undef>"]);
...
if ismember('Trial', T.Properties.VariableNames)
    ...
    TT = attach_mouselevel_metadata_to_keys(TT, cohort, cohortMap);
    TT.Period      = categorical(period_label_vec(TT.day_index, false, unreliableDays), ...
                                 ["Pre","During","Post","Withdrawal","Reexposure","<undef>"]);
    TT.PeriodClean = categorical(period_label_vec(TT.day_index, true,  unreliableDays), ...
                                 ["Pre","During","Post","Withdrawal","Reexposure","<excluded>","<undef>"]);
    writetable(TT, fullfile(outDir,'per_trial_features.csv'));
end

%% choose which bin column to use for plots
if USE_CLEAN_BINS_FOR_PLOTS
    binColSess = 'PeriodClean';
    binColTT   = 'PeriodClean';
else
    binColSess = 'Period';
    binColTT   = 'Period';
end
...
if opts.A_core
    do_core_rates_and_survival(TT, Sess, outDir, RHYTHM_BAND, PLOT_MAX_MOUSE_EXAMPLES, binColSess, binColTT);
end
if opts.B_micro
    do_microstructure_block(Sess, TT, outDir, BOUT_WITHIN_IEI, BOUT_GAP_IEI, PLOT_MAX_MOUSE_EXAMPLES, binColSess);
end
if opts.C_rhythm
    do_rhythm_block(T, tb, Sess, TT, outDir, BIN_50MS, RHYTHM_BAND, PEAK_SEARCH, PHASE_BAND, rewardCol, binColSess);
end
if opts.D_psth
...
if opts.UNSUP_engineer || opts.UNSUP_reduce || opts.UNSUP_cluster || opts.UNSUP_consensus
    U = engineer_unsup_features(Sess, TT, outDir, binColSess);
end
...
%% ========================= NEW COHORT HELPERS ===========================
function cohort = build_new_cohort_table()
    % CanonKey must be lowercase "#### color"
    rows = {
        "6100 red",    "6100","red",    "f","Passive", 1
        "6100 orange", "6100","orange", "f","Passive", 1
        "6100 black",  "6100","black",  "f","Active",  1

        "0911 red",    "0911","red",    "f","Active",  2
        "0911 orange", "0911","orange", "f","Passive", 2

        "0911 black",  "0911","black",  "f","Passive", 3
        "0911 white",  "0911","white",  "f","Active",  3

        "0910 red",    "0910","red",    "m","Passive", 4
        "0910 orange", "0910","orange", "m","Passive", 4
        "0910 black",  "0910","black",  "m","Active",  4

        "6099 red",    "6099","red",    "m","Passive", 5
        "6099 orange", "6099","orange", "m","Active",  5

        "6099 black",  "6099","black",  "m","Active",  6
        "6099 white",  "6099","white",  "m","Passive", 6
    };
    cohort = cell2table(rows, 'VariableNames', {'CanonKey','Cage','Color','Sex','Group','PairID'});
    cohort.CanonKey = lower(string(cohort.CanonKey));
    cohort.Cage     = string(cohort.Cage);
    cohort.Color    = lower(string(cohort.Color));
    cohort.Sex      = lower(string(cohort.Sex));
    cohort.Group    = string(cohort.Group);
    cohort.PairID   = double(cohort.PairID);
end

function [T, summary] = attach_cohort_metadata(T, cohort, cohortMap)
    n = height(T);
    CanonKey = strings(n,1);
    Cage     = strings(n,1);
    Color    = strings(n,1);
    Sex      = strings(n,1);
    Group    = strings(n,1);
    ...
    for i=1:n
        mk = string(T.mouse_key(i));
        ck = canonical_mouse_key(mk);
        CanonKey(i) = ck;
        if ck~="" && isKey(cohortMap, ck)
            row = cohortMap(ck);
            Cage(i)  = cohort.Cage(row);
            Color(i) = cohort.Color(row);
            Sex(i)   = cohort.Sex(row);
            Group(i) = cohort.Group(row);
            PairID(i)= cohort.PairID(row);
        else
            Cage(i)  = "";
            Color(i) = "";
            Sex(i)   = "";
    ...
    T.Color    = Color;
    T.Sex      = Sex;
    % Group categorical with known levels; unknown -> "<unknown>"
    g = Group; g(g=="") = "<unknown>";
    T.Group    = categorical(g, ["Active","Passive","<unknown>"]);
    T.PairID   = PairID;

    % summary table
    mice = unique(T(:,{'mouse_key','CanonKey','Group','Sex','PairID'}),'rows','stable');
    assigned = mice.CanonKey ~= "";
    summary = table();
    summary.n_unique_mouse_key = height(mice);
    summary.n_assigned         = nnz(assigned);
    summary.n_unassigned       = nnz(~assigned);
    summary.unassigned_list    = join(string(mice.mouse_key(~assigned)), "; ");
end

function K = canonical_mouse_key(mouse_key)
    % Robustly parse "#### color" from mouse_key strings like:
    %   "6100_red_f_s", "6100 red", "0910 orange m_p", etc.
    s = lower(string(mouse_key));
    s = regexprep(s, '[_\-]+', ' ');
    s = regexprep(s, '\s+', ' ');
    s = strtrim(s);

    cage = regexp(s, '\d{4}', 'match', 'once');
    if isempty(cage), K=""; return; end

    colors = ["red","orange","black","white"];
    col = "";
    for c = colors
        if ~isempty(regexp(s, ['\<' char(c) '\>'], 'once'))
            col = c; break;
        end
    end
    if col==""
        % sometimes color glued (rare); fall back to contains
        for c = colors
            if contains(s, c)
                col = c; break;
            end
    ...
    if col==""
        ...
        K = lower(string(cage) + " " + string(col));
    end
end

function Tkeys = attach_mouselevel_metadata_to_keys(Tkeys, cohort, cohortMap)
    % Adds: CanonKey, Cage, Color, Sex, Group, PairID to any table with mouse_key
    Tkeys = ensureString(Tkeys,'mouse_key');
    n = height(Tkeys);
    CanonKey = strings(n,1);
    Cage     = strings(n,1);
    Color    = strings(n,1);
    ...
    for i=1:n
        ck = canonical_mouse_key(Tkeys.mouse_key(i));
        CanonKey(i) = ck;
        if ck~="" && isKey(cohortMap, ck)
            row = cohortMap(ck);
            Cage(i)  = cohort.Cage(row);
    ...
    Tkeys.CanonKey = CanonKey;
    Tkeys.Cage     = Cage;
    Tkeys.Color    = Color;
    Tkeys.Sex      = Sex;
    Tkeys.Group    = categorical(Group, ["Active","Passive","<unknown>"]);
    Tkeys.PairID   = PairID;
end

function labs = period_label_vec(d, useClean, unreliableDays)
    % Map day -> phase label. If useClean, mark unreliableDays as "<excluded>".
    labs = repmat("<undef>", numel(d), 1);

    labs(d>=3  & d<=5 ) = "Pre";
    labs(d>=6  & d<=10) = "During";
    labs(d>=11 & d<=13) = "Post";
    labs(d>=14 & d<=16) = "Withdrawal";
    labs(d>=17 & d<=18) = "Reexposure";

    if useClean
        bad = ismember(d, unreliableDays);
        labs(bad & labs~="<undef>") = "<excluded>";
    end
end
...
function tb = pickTimebase(T)
    ...
    for i=1:numel(cands)
        if ismember(cands{i}, T.Properties.VariableNames)
            ...
            if any(isfinite(v))
                tb = v;
                if strcmpi(cands{i},'Frame'), tb = v/30; end
                return
            end
        end
    end
...
function col = pickRequirement(T)
    ...
    col = '';
    for i=1:numel(cands)
        if ismember(cands{i},T.Properties.VariableNames), col=cands{i}; return; end
    end
end
...
function F = compute_session_features_full(t, l, thrWithin, thrGap)
    ...
    if numel(on)>=2
        ...
        k=2; c=0;
        while k<=numel(gap)
            if ~inBout && gap(k)<thrWithin
                inBout=true; bc=bc+1; c=1; wbIEI=[wbIEI; gap(k)];
            elseif inBout
                if gap(k)<thrWithin
                    c=c+1; wbIEI=[wbIEI; gap(k)];
                elseif gap(k)>=thrGap
                    inBout=false; bbGap=[bbGap; gap(k)];
                end
            end
            if ~inBout && c>0
                lick_in_bout=lick_in_bout+(c+1); c=0;
            end
            k=k+1;
        end
        ...
        pfit = polyfit(log10(wins(~isnan(AF))), log10(AF(~isnan(AF))), 1);
        F.allan_slope = pfit(1);
    end
end
...
function do_core_rates_and_survival(TT, Sess, outDir, rband, MAXEX, binColSess, binColTT)
    plot_group_bin_violin(Sess, 'licks_per_min', 'Licks / min', outDir, 'A_core_violin_licks_per_min.png', binColSess);
    ...
    if ~isempty(TT)
        ...
        writetable(TT(:,{'mouse_key','day_index','session_idx','Trial','licks_per_reward','wasted_fraction','Group','PairID'}), ...
            fullfile(outDir,'A_core_licks_per_reward_and_wasted.csv'));

        plot_group_bin_violin(TT, 'licks_per_reward', 'Licks / reward', outDir, 'A_core_violin_licks_per_reward.png', binColTT);
        plot_group_bin_violin(TT, 'wasted_fraction',  'Wasted lick fraction', outDir, 'A_core_violin_wasted_fraction.png', binColTT);
...
function plot_group_bin_violin(Tin, col, ylab, outDir, fname, binCol)
    if ~ismember(binCol,Tin.Properties.VariableNames) || ~ismember('Group',Tin.Properties.VariableNames), return; end
    if ~ismember(col, Tin.Properties.VariableNames), return; end

    bins = categories(Tin.(binCol));
    bins(endsWith(bins,"<undef>")) = [];
    bins(endsWith(bins,"<excluded>")) = []; % don’t plot excluded as a bin by default
    G = {"Active","Passive"};

    fig = figure('Color','w','Position',[90 90 1040 420]); 
    tl=tiledlayout(1,numel(bins),'TileSpacing','compact','Padding','compact');

    for b=1:numel(bins)
        nexttile; hold on
        for g=1:numel(G)
            y = Tin.(col)(Tin.(binCol)==bins{b} & Tin.Group==categorical(G{g},["Active","Passive","<unknown>"]));
            y = y(isfinite(y));
            if isempty(y), continue; end
            [kde_x,kde_y] = kde1(y, 200);
            kde_y = kde_y / max(kde_y) * 0.35;
            if g==1
                patch([kde_x, fliplr(kde_x)],[kde_y, -fliplr(kde_y)],[0.8 0.85 1],'EdgeColor','none','FaceAlpha',0.6);
            else
                patch([kde_x, fliplr(kde_x)],[kde_y, -fliplr(kde_y)],[1 0.85 0.8],'EdgeColor','none','FaceAlpha',0.6);
            end
            plot(median(y), 0, 'kx', 'MarkerSize',8,'LineWidth',1.6);
        end
        ylabel(ylab); title(bins{b}); box on; grid on
    end
    title(tl, sprintf('%s by Group × Period', ylab));
...
function km_by_requirement(TT, outDir)
    ...
    for r = reqLevels
        ...
        for g=1:numel(groups)
            x = double(sub.time_to_reward(sub.Group==categorical(groups{g},["Active","Passive","<unknown>"])));
            x = x(isfinite(x) & x>0);
        ...
        A = double(sub.time_to_reward(sub.Group=="Active"));  A=A(isfinite(A)&A>0);
        P = double(sub.time_to_reward(sub.Group=="Passive")); P=P(isfinite(P)&P>0);
        if numel(A)>=3 && numel(P)>=3
            pLR = logrank_p(A,P);
            writetable(table(r, numel(A), numel(P), pLR, 'VariableNames',{'requirement','nActive','nPassive','p_logrank'}), ...
...
function plot_efficiency_vs_requirement(TT, outDir)
    ...
    gP = (sub.Group=="Passive");
    scatter(sub.requirement(~gP), sub.licks_before_reward(~gP), 12, 'filled','MarkerFaceAlpha',0.5);
    scatter(sub.requirement(gP),  sub.licks_before_reward(gP),  12, 'filled','MarkerFaceAlpha',0.5);
    ...
    title('Efficiency vs requirement'); grid on; box on; legend({'Active','Passive','perfect'},'Location','best');
...
function spaghetti_per_mouse(Sess, metric, outDir, MAXEX, fname)
    ...
    for i=1:numel(ms)
        r = (string(Sess.mouse_key)==string(ms(i)));
...
function do_microstructure_block(Sess, TT, outDir, thrWithin, thrGap, MAXEX, binColSess)
    for v = ["iei_median","iei_cv","iei_lv","cv2_median","iei_entropy","bouts_per_min","duty_cycle","mean_licks_per_bout","within_bout_iei","between_bout_gap","fano_5s","allan_slope"]
        plot_group_bin_violin(Sess, char(v), strrep(char(v),'_','\_'), outDir, ['B_micro_violin_' char(v) '.png'], binColSess);
    end
...
function bout_rasters_examples(Sess, outDir, MAXEX, thrWithin, thrGap)
    ...
    tl=tiledlayout(ceil(numel(ord)/3),3,'TileSpacing','compact','Padding','compact');
    for i=1:numel(ord)
        nexttile; hold on
        title(sprintf('%s d%d s%d', string(Sess.mouse_key(ord(i))), Sess.day_index(ord(i)), Sess.session_idx(ord(i))));
        % cannot reconstruct full IEI without saving lick times; leave placeholder
        yline(thrWithin,'r--'); yline(thrGap,'k:');
        ylabel('IEI (s)'); xlabel('consecutive IEI idx'); grid on; box on
    end
    title(tl,'Bout raster thresholds (placeholder; lick-times not stored at session level)');
    savepng_local(fig, fullfile(outDir,'B_micro_bout_rasters_examples.png')); close(fig);
end

function per_trial_iei_cdf(TT, outDir)
    fig = figure('Color','w','Position',[100 100 600 460]); hold on
    for g = ["Active","Passive"]
        y = double(TT.iei_median(TT.Group==categorical(g,["Active","Passive","<unknown>"])));
        y=y(isfinite(y));
        if isempty(y), continue; end
        [f,x] = ecdf(y);
...
%% ===================== 2C RHYTHM BLOCK =================================
function do_rhythm_block(T, tb, Sess, TT, outDir, BIN, band, peakRange, phaseBand, rewardCol, binColSess)
    [specTbl, specDir] = spectra_per_session(T, tb, Sess, outDir, BIN);
    S = summarize_spectra(specTbl, band, peakRange);
    S = attach_mouselevel_metadata_to_keys(S, unique(Sess(:,{'CanonKey','Cage','Color','Sex','Group','PairID'}),'rows','stable'), containers.Map(), true); %#ok<NASGU>
    writetable(S, fullfile(outDir,'C_rhythm_spectral_metrics.csv'));
    % Note: spec summary doesn’t include Period; skip group×period violins here unless you join back on session keys.
end
...
function [specTbl, specDir] = spectra_per_session(T, tb, Sess, outDir, BIN)
    ...
    rows = {};
    for i=1:height(Sess)
        r = (string(T.mouse_key)==string(Sess.mouse_key(i))) & (T.day_index==Sess.day_index(i)) & (T.session_idx==Sess.session_idx(i));
        t = double(tb(r)); l = logical(T.Lick_TTL(r));
    ...
    if isempty(rows), specTbl = table(); return; end
...
function S = summarize_spectra(specTbl, band, peakRange)
    ...
    for i=1:height(specTbl)
        ...
        [pk,ix] = max(P(pr)); ff = f(pr);
        if isempty(ix), pk=NaN; pkHz=NaN; FWHM=NaN;
        else
            pkHz = ff(ix);
...
function [Pstart, Prew] = psth_group(T, tb, TT, BIN, rewardCol)
    ...
    for g = 1:2
        sub = TT(TT.Group==categorical(groups(g),["Active","Passive","<unknown>"]),:);
        ...
        for i = 1:height(sub)
            r = (string(T.mouse_key)==string(sub.mouse_key(i))) & (T.day_index==sub.day_index(i)) & ...
    ...
    if isempty(rowsS), Pstart = table();
    else, Pstart = cell2table(rowsS, 'VariableNames',{'Group','t','mean_rate','sem_rate'});
    end

    if isempty(rowsR), Prew = table();
    else, Prew = cell2table(rowsR, 'VariableNames',{'Group','t','mean_rate','sem_rate'});
    end
end

function D = ramp_pause_metrics(Prew)
    if isempty(Prew) || ~ismember('Group',Prew.Properties.VariableNames), D=table(); return; end
    if ~iscategorical(Prew.Group), Prew.Group = categorical(string(Prew.Group)); end

    rows={};
    for g = categories(Prew.Group)'
        sub = Prew(Prew.Group==g,:); if isempty(sub), continue; end
        t = double(sub.t{1}); y = double(sub.mean_rate{1});
        mask = t>=-1.5 & t<=0; 
        if nnz(mask)>=2
            p = polyfit(t(mask),y(mask),1);
            slope = p(1);
        else
            slope = NaN;
        end
    ...
    if isempty(rows), D=table();
    else, D = cell2table(rows,'VariableNames',{'Group','ramp_slope','pause_depth','pause_duration'});
    end
end
...
function hazard_curves(TT, HAZ, outDir)
    ...
    for r = reqLevels
        ...
        h = d./max(n,1);
        plot(edges(1:end-1), h, '-o', 'DisplayName', sprintf('req=%g', r));
    end
...
function do_prediction_block(Sess, TT, outDir, SEED)
    rng(SEED);
    F = Sess(:,{'mouse_key','day_index','session_idx','Group','PairID',...
        'licks_per_min','iei_cv','iei_lv','cv2_median','iei_entropy','bouts_per_min','duty_cycle','fano_5s','allan_slope'});

    y = double(F.Group=="Passive");
    X = zscore_tbl(F(:,6:end));
...
function beta = glmnet_cd(X, y, lam, alpha)
    ...
    for it=1:200
        ...
        if max(abs(beta))<1e-9, break; end
    end
end
...
function sc = predict_logit(Xtr,ytr,Xte)
    ...
    b = glmnet_cd(Xtr,ytr,0.01,0.0);
    sc = 1./(1+exp(-Xte*b));
end

%% ===================== 3) UNSUPERVISED: features/reduce/cluster =========
function U = engineer_unsup_features(Sess, TT, outDir, binColSess)
    D1 = Sess(:,{'mouse_key','day_index','session_idx','Group','PairID',binColSess,...
        ...
        for b=1:B
            ...
            for i=1:numel(samp)
                for j=i+1:numel(samp)
                    if idx(i)==idx(j)
                        M(samp(i),samp(j)) = M(samp(i),samp(j))+1; 
                        M(samp(j),samp(i)) = M(samp(i),samp(j));
                    end
                end
            end
        end
...
function req_align_and_hazard(TT, outDir)
    ...
    rows = {};
    for i = 1:height(G)
        r = (string(TT.mouse_key)==string(G.mouse_key(i))) & (TT.day_index==G.day_index(i)) & (TT.session_idx==G.session_idx(i));
        req = double(TT.requirement(r));
        eff = double(TT.efficiency(r));
        good = isfinite(req) & isfinite(eff);
        if nnz(good) >= 2
            p = polyfit(req(good), eff(good), 1);
            slope = p(1);
        else
            slope = NaN;
        end
...
function do_mixed_models_approx(TT, Sess, outDir)
    if ~isempty(TT)
        y = double(TT.licks_before_reward);
        req = double(TT.requirement);
        grp = double(TT.Group=="Passive");
        M = isfinite(y) & isfinite(req);
        y=y(M); req=req(M); grp=grp(M);
    ...
    if ~isempty(TT)
        ...
        y = y(M); req = double(TT.requirement(M)); grp = double(TT.Group(M)=="Passive");
        X = [ones(numel(y),1), zscore_vec(req), grp, zscore_vec(req).*grp];
...
function save_cluster_plot(Ured, idx, outDir, K)
    ...
    gscatter(Ured.PC1,Ured.PC2,idx);
    xlabel('PC1'); ylabel('PC2'); title(sprintf('k-means K=%d',K)); grid on; box on
...
function [x,y] = kde1(v, n)
    v=v(isfinite(v)); 
    if numel(v)<3
        x=linspace(min(v)-eps,max(v)+eps,20); y=ones(size(x)); return; 
    end
