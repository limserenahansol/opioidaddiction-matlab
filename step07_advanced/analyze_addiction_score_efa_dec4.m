function analyze_addiction_score_efa_dec4()
...
% Paper-style mouse-level EFA + projection (separate subfolder)
outDirMouse = fullfile(outDir, 'mousefit_projection');
if ~exist(outDirMouse,'dir'), mkdir(outDirMouse); end
fidStart = fopen(fullfile(outDirMouse,'mousefit_projection_note.txt'),'w');
if fidStart > 0
    fprintf(fidStart,'Mousefit projection started.\n');
    fclose(fidStart);
end
outMouse = computeAddictionMouseLevelEFA(D, useVars, outDirMouse);
if ~isempty(outMouse)
    plotAddictionSpaghetti(outMouse, outDirMouse);
    plotAddictionSpaghettiZ(outMouse, outDirMouse);
    outMouse.Period = periodOfDay(double(outMouse.day_index));
    plotAddictionByPhase(outMouse, outDirMouse);
    plotAddictionByPhaseZ(outMouse, outDirMouse);
    plotAddictionDeltaFromPre(outMouse, outDirMouse);
    plotAddictionRawGroupCompare(outMouse, outDirMouse);
else
    fid = fopen(fullfile(outDirMouse,'mousefit_projection_note.txt'),'a');
    if fid > 0
        fprintf(fid,'Mousefit projection produced no outputs. Check feature/mouse filtering.\n');
        fclose(fid);
    end
end
...
function outMouse = computeAddictionMouseLevelEFA(D, useVars, outDir)
% Mouse-level EFA fit, then project to mouse×day
outMouse = table();
noteFile = fullfile(outDir,'mousefit_projection_note.txt');
fidNote = fopen(noteFile,'w');
if fidNote <= 0
    return
end
fprintf(fidNote,'Mousefit projection notes\n');

% Build mouse-level matrix (median across days)
mk = unique(D.mouse_key,'stable');
...
% Drop features with >70% missing (relaxed to avoid empty output)
missFeat = mean(~isfinite(Xmouse),1);
keepFeat = missFeat <= 0.70;
Xmouse = Xmouse(:, keepFeat);
useVars = useVars(keepFeat);
if size(Xmouse,2) < 4
    fprintf(fidNote,'Too few features after missingness filter; using top-variance features.\n');
...
if size(Xmouse,2) < 2 || size(Xmouse,1) < 3
    fprintf(fidNote,'Insufficient data after filtering: nMice=%d nFeat=%d\n', size(Xmouse,1), size(Xmouse,2));
    fclose(fidNote);
    return
end

% Drop mice with >70% missing (relaxed)
...
% Rank check to avoid non-PD covariance
R = corrcoef(Xz);
R = (R + R')/2;
eigR = eig(R);
rankR = sum(eigR > 1e-6);
fprintf(fidNote,'Rank estimate: %d (nMice=%d, nFeat=%d)\n', rankR, size(Xz,1), size(Xz,2));
if rankR < 2
    fprintf(fidNote,'Rank too low for EFA after filtering.\n');
    fclose(fidNote);
    return
end

% Choose number of factors by common variance >=85%
targetExpl = 0.85;
maxFactors = min([12, size(Xz,2)-1, size(Xz,1)-1, rankR-1]);
best = struct('nf',[],'Lambda',[],'Psi',[],'commonExpl',-Inf);
for nf=1:maxFactors
    [ok, Lambda, Psi, msg] = tryFactoranMouse(Xz, nf);
    if ~ok
    ...
    if commonExpl > best.commonExpl
        best.nf = nf; best.Lambda = Lambda; best.Psi = Psi; best.commonExpl = commonExpl;
    end
    if commonExpl >= targetExpl
        break
    end
end
if isempty(best.nf)
    fprintf(fidNote,'EFA failed for all nf up to %d.\n', maxFactors);
    fclose(fidNote);
    return
elseif best.commonExpl < targetExpl
    fprintf(fidNote,'Target common variance not reached; using nf=%d (%.1f%%).\n', ...
        best.nf, 100*best.commonExpl);
end

Lambda = best.Lambda; Psi = best.Psi; nf = best.nf;

% Mouse-level factor scores
...
outMouse = Dsub(:, {'mouse_key','day_index','day_name','Group'});
outMouse.AddictionScore = AI_day;
outMouse.AddictionScore_z = AI_day_raw;
for k=1:nf
    outMouse.("Factor"+k) = Fday(:,k);
end

writetable(outMouse, fullfile(outDir,'addiction_score_mousefit_by_mouse_day.csv'));
writetable(mouseTbl, fullfile(outDir,'addiction_score_mousefit_by_mouse.csv'));
fprintf(fidNote,'Success: nf=%d, nMice=%d, nFeat=%d\n', nf, size(Xmouse,1), numel(useVars));
fclose(fidNote);
end
...
try
    ...
    msg = [msg ' | R+PD: ' ME.message];
end
end

function Rpd = makePosDef(R, epsVal)
[V,D] = eig(R);
d = diag(D);
d(d < epsVal) = epsVal;
Rpd = V * diag(d) * V';
