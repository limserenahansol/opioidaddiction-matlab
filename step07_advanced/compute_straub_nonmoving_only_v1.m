...
if nargin >= 1 && strcmp(vname, 'STRAUB_Pct_Non_moving')
    ylbl = 'Straub tail (%) — (time with tail >30° / 5 min observation) × 100';
else
    ylbl = 'Straub tail frames (non-moving)';
end
end
...
fh = figure('Color','w','Position',[90 90 1050 420]);
...
sgtitle(sprintf('Straub tail in Active and Passive mice (both sexes) — %s trajectories', vname));
...
fh = figure('Color','w','Position',[90 90 1050 430]);
tiledlayout(1,2,'TileSpacing','compact','Padding','compact');
...
sgtitle('Straub tail: phase comparison within each group (measured During & Withdrawal)');
...
fh = figure('Color','w','Position',[90 90 1200 420]);
...
sgtitle('Straub tail: Active vs Passive within each phase (both sexes, equivalent morphine exposure)');
...
fh = figure('Color','w','Position',[90 90 900 650]);
