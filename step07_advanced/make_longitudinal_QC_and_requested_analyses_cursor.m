function make_longitudinal_QC_and_requested_analyses_cursor()
...
P.value = P.value(isfinite(P.value));
...
try
    ...
    lines{end+1} = sprintf('Kruskal-Wallis p=%.4g', pKw); %#ok<AGROW>
catch
    return;
end
periods = categories(P.period);
...
for i=1:numel(periods)
    for j=i+1:numel(periods)
        ...
        pval = ranksum(a, b);
        lines{end+1} = sprintf('%s vs %s p=%.4g', periods{i}, periods{j}, pval); %#ok<AGROW>
    end
end
end
...
for j=1:numel(daysAll)
    ...
    pval = ranksum(a, p);
    lines{end+1} = sprintf('Day %d p=%.4g', d, pval); %#ok<AGROW>
end
end
...
txt = strjoin(lines, '\n');
annotation(ax.Parent, 'textbox', [0.68 0.60 0.30 0.30], ...
